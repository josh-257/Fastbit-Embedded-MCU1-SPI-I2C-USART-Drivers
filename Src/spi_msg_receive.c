...
// Your working hardware trace definitions
...
{
    // Enable hardware traces
    DEMCR |= (1 << 24);
    ITM_TRACE_EN |= (1 << 0);

    for (int i = 0; i < len; i++)
    {
        // Your proven working hardware loop
        while (!(ITM_STIMULUS_PORT0 & 1));
        ITM_STIMULUS_PORT0 = (uint8_t)(*ptr++);
    }
    return len;
}


volatile uint8_t txRxComplete = 0;
...
//volatile uint8_t txRxComplete = 0; //added
...
int main(void)
{
	...
	*/
	SPI_SSOEConfig(SPI2,ENABLE);
	 ...
	 GPIO_IRQPriorityConfig(IRQ_NO_EXTI9_5, 15); // Set Arduino trigger to lowest priority
	 GPIO_IRQPriorityConfig(IRQ_NO_SPI2, 5);
	...
	while(1){
		...
		while(!rcvStop)
		{
		    ...
		    while (SPI_ReceiveIT(&SPI2handle, (uint8_t*)&ReadByte, 1) == SPI_BUSY_IN_RX);
